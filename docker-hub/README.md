...
#### Image Contents
- **vLLM** built from upstream `main` at `4868312128` (the `0.30` branch,
  ...
  upstream: #55222 (right-sizes the indexer prefill workspace — without it,
  #55221 cuts the auto-fit `max_model_len` to 516K, losing the 1M context)
...
- **FlashInfer** via the `flashinfer-jit-cache==0.6.18.post1` wheel from the flashinfer.ai index (upstream's own pin), plus `set_autotune_process_group` (Xid-69 fix)
